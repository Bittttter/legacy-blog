...
tags: ['nodejs', 'network', 'asynchronous']
published: true
cover: ./cover.jpg
coverCredit: Photo by ArtisanalPhoto on Unsplash
---
...
The event loop also handles timer events, i.e. `setTimeout`, `setInterval`, and `setImmediate`. When a timer event is registered, the event loop will wait specified amount of time and trigger the callback.
...
![Event Loop Diagram](./illustration.jpg)
