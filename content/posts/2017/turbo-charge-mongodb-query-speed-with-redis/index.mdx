...
tags: ['MongoDB', 'Redis', 'Performance']
published: true
cover: ./cover.jpg
...
To work with Redis in nodeJS, we need to install the `redis` npm module.
...
mongoose.Query.prototype.exec = async function() {
  ...
  if (cacheValue) {
    ...
    // Notice that if the cached value is an array,
...
Now, in the route controller, we can call the `cache()` method on the mongoose query object and pass in the hash key.
...
module.exports = async (req, res, next) => {
  // By making this middleware async,
